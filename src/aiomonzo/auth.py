...
class RefreshLockFactory(Protocol):
    """Create an async lock context for one logical OAuth token set."""

    def __call__(self) -> AbstractAsyncContextManager[None]:
        """Return a context manager that serializes refresh-token rotation."""


class _LocalRefreshLockFactory:
    """Serialize refreshes within one process by default."""
...
class OAuthAccessTokenProvider:
    ...
    def __init__(
        ...
        *,
        ...
        refresh_lock_factory: RefreshLockFactory | None = None,
        ...
        self._refresh_lock_factory = refresh_lock_factory or _LocalRefreshLockFactory()
    ...
    async def _refresh(self, *, expected_access_token: str) -> OAuthToken:
        async with self._refresh_lock_factory():
